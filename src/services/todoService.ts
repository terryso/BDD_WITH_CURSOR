...
interface ITodoService {
  add(options: AddTodoOptions): Todo;
...
export class TodoService implements ITodoService {
  ...
  constructor(options: TodoServiceOptions | string = {}) {
    if (typeof options === 'string') {
      options = { dataFile: options };
    }
    this.inMemory = options.inMemory || false;
    this.storageFile = options.dataFile || path.join(process.cwd(), '.todo-data.json');
  ...
  private sortByDeadline(todos: Todo[], order: 'asc' | 'desc'): Todo[] {
    // 首先按完成状态分组
